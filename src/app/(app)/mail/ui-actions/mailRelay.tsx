  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent
        ...
        className={cn(
          css({ p: "large", fontWeight: "bold" }),
          stack({ direction: "col", gap: "lg" }),
        )}
      >
        <DialogTitle
          className={cn(css({ fontSize: "xlarge", color: "text2" }))}
        >
          Mail Relay
        </DialogTitle>
        <div
    ...
    <Dialog>
      <DialogTrigger>
        <Card
          ...
          className={cn(
            stack({ justify: "between", direction: "col", gap: "xl" }),
            css({
              p: "medium",
        ...
        >
          <div className={cn(stack({ gap: "md" }))}>
            {isWebsite && (
              <Image
                width={40}
                height={40}
                src={`https://www.google.com/s2/favicons?domain=${item.label}&sz=256`}
          ...
          <div className={cn(stack({ justify: "between" }))}>
            <span
              className={cn(css({ color: "text1", p: "small" }))}
      ...
      >
        <DialogTitle className={cn(css({ fontSize: "large", color: "text2" }))}>
          {item.label}
        </DialogTitle>
        testing
      </DialogContent>
