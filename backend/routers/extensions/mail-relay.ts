...
export const mailRelayRouter = router({
  ...
  removeAlias: protectedProc.input(z.object({
    alias: z.string(),
    enabled: z.boolean()
  ...
  createAlias: protectedProc.input(z.object({
    label: z.string()
  })).mutation(async ({ctx, input}) => {
    ...
    const existingAlias = await ctx.dyn.send(getCommand)
    .then(v=>v.Items)
    .then(v => addressAliasInterface.array().parse(v));

    if (existingAlias.length > 0) {
      throw new TRPCError({code: "CONFLICT", message: "Alias already exists"})
    }
